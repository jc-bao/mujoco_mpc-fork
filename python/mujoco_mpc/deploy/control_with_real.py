...
class Controller:
    def __init__(
        ...
        self.qd_buffer_lp = np.zeros(
            (self.low_pass_filter_window_size, self.config.nqd_ctrl)
        )
        self.q_buffer_lp = np.zeros(
            (self.low_pass_filter_window_size, self.config.nq_ctrl)
        )
    ...
    def init_stand_go2(self):
        while self.config.percent_3 < 1:
            ...
            if (self.config.percent_1 == 1) and (self.config.percent_2 <= 1):
                ...
                for i in range(self.config.nu_real):
                    self.low_cmd_msg.motor_cmd[i].q = (
                        1 - self.config.percent_2
                    ) * self.config._targetPos_1[
                        i
                    ] + self.config.percent_2 * self.config._targetPos_2[
                        i
                    ]
                    self.low_cmd_msg.motor_cmd[i].dq = 0
                    self.low_cmd_msg.motor_cmd[i].kp = self.config.kp_real[i]
            ...
            if self.config.control_mode == "torque":
                ...
                if (
                    (self.config.percent_1 == 1)
                    and (self.config.percent_2 == 1)
                    and (self.config.percent_3 < 1)
                ):
            ...
            if self.config.control_mode == "position" and np.isclose(
                self.config.percent_1, 1
            ):
                break
        print("Stand up complete")
    ...
    def init_stand_h1(self):
        ...
        start_pos_sim = h1_joint_remapping(
            self.config.motor_order, start_pos, direction="real2sim"
        )
    ...
    def set_action_h1(self, ctrl):
        if self.config.control_mode == "torque":
            ...
            q_des = (
                ctrl * self.global_ctrl_scale
                + (1 - self.global_ctrl_scale) * self.config.q_default
            )
        self.low_cmd_msg.head[0] = 0xFE
        self.low_cmd_msg.head[1] = 0xEF
        self.low_cmd_msg.level_flag = 0xFF
        ...
        if np.allclose(ctrl, np.zeros(self.config.nu_real - 1), atol=1e-3):
            ...
            for i in range(self.config.nu_real):
                ...
                if i in self.config.locked_joint_idx:
                    ...
                    if j >= 9:
                        self.low_cmd_msg.motor_cmd[i].kp = (
                            self.config.kp_real[j] * self.global_kp_scale_upper
                        )
                        self.low_cmd_msg.motor_cmd[i].kd = (
                            self.config.kd_real[j] * self.global_kd_scale_upper
                        )
                    else:
                        self.low_cmd_msg.motor_cmd[i].kp = (
                            self.config.kp_real[j] * self.global_kp_scale_lower
                        )
                        self.low_cmd_msg.motor_cmd[i].kd = (
                            self.config.kd_real[j] * self.global_kd_scale_lower
                        )
                else:
        ...
        if np.allclose(ctrl, np.zeros(self.config.nu_real), atol=1e-3):
            ...
            for i in range(self.config.nu_real):
                if i in self.config.locked_joint_idx:
                    ...
                    print(
                        f"Control Command sent to motor index {i}: q_des={q_des[i]}, tau={tau[i]}, kp={self.config.kp_real[i]}, kd={self.config.kd_real[i]}"
                    )
                    self.low_cmd_msg.motor_cmd[i].q = q_des[i]
                    self.low_cmd_msg.motor_cmd[i].tau = tau[i]
                    self.low_cmd_msg.motor_cmd[i].kp = (
                        self.config.kp_real[i] * self.global_kp_scale
                    )
                    self.low_cmd_msg.motor_cmd[i].kd = (
                        self.config.kd_real[i] * self.global_kd_scale
                    )
        self.low_cmd_msg.crc = self.crc.Crc(self.low_cmd_msg)
        self.low_cmd_publisher.Write(self.low_cmd_msg)
    ...
    def get_state(self):
        ...
        if self.robot_name == "h1" or self.robot_name == "h1_mani":
            q_jnt = h1_joint_remapping(
                self.config.motor_order, self.q[7:], direction="real2sim"
            )
            # print("q_jnt", q_jnt)
            qd_jnt = h1_joint_remapping(
                self.config.motor_order, self.qd[6:], direction="real2sim"
            )
        ...
        if "torso" in self.config.vicon_object_name:
            ...
            R_pelvis_torso = R.from_euler(
                "xyz", [0.0, 0.0, torso_joint_angle], degrees=False
            )
    ...
    def main_loop(self):
        ...
        if self.robot_name == "h1_2" or self.robot_name == "h1_2_simple":
            left_foot_geom_names = [
                "left_heel_left",
                "left_heel_right",
                "left_toe_left",
                "left_toe_right",
            ]
            right_foot_geom_names = [
                "right_heel_left",
                "right_heel_right",
                "right_toe_left",
                "right_toe_right",
            ...
            left_foot_geom_idx = [
                ...
                for name in left_foot_geom_names
            ]
            right_foot_geom_idx = [
                mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_GEOM, name)
                for name in right_foot_geom_names
            ]
        ...
        try:
            with agent_lib.Agent(
                ...
                while True:
                    ...
                    self.global_rpy_offset = np.array(
                        [
                            bars["roll_offset"].get(),
                            bars["pitch_offset"].get(),
                            bars["yaw_offset"].get(),
                        ]
                    )
                    ...
                    # low pass filter
                    qd_sim_lp = (
                        self.low_pass_filter_gamma * qd_sim
                        + (1 - self.low_pass_filter_gamma) * self.qd_buffer_lp[-1]
                    )
                    q_sim_lp = (
                        self.low_pass_filter_gamma * q_sim
                        + (1 - self.low_pass_filter_gamma) * self.q_buffer_lp[-1]
                    )
                    ...
                    if bars["calibrate"].get() > 0.5:
                        ...
                        if (
                            self.robot_name == "h1_2"
                            or self.robot_name == "h1_2_simple"
                        ):
                            data.qpos = q_sim
                            data.qvel = qd_sim
                            ...
                            left_foot_geom_pos = [
                                data.geom_xpos[idx] for idx in left_foot_geom_idx
                            ]
                            left_foot_geom_mat = [
                                data.geom_xmat[idx] for idx in left_foot_geom_idx
                            ]
                            right_foot_geom_pos = [
                                data.geom_xpos[idx] for idx in right_foot_geom_idx
                            ]
                            right_foot_geom_mat = [
                                data.geom_xmat[idx] for idx in right_foot_geom_idx
                            ]
                            ...
                            rot_body_origin = R.from_euler(
                                "xyz", self.global_rpy_offset, degrees=False
                            )
                            foot_rot_origin = foot_rot * rot_body_origin.inv()
                            # get rpy with inverse of foot_rot
                            self.global_rpy_offset = foot_rot_origin.inv().as_euler(
                                "xyz", degrees=False
                            )
                            ...
                            self.robot_name == "go2"
                            or self.robot_name == "h1"
                            or self.robot_name == "h1_mani"
                        ):
                            ...
                            R_marker_body = R.from_euler(
                                "xyz", self.global_rpy_offset, degrees=False
                            )
                            R_world_marker = R_world_body * R_marker_body.inv()
                            q_sim_marker[3:7] = R_world_marker.as_quat(
                                scalar_first=True
                            )
                            # undo qd
                            ...
                            foot_geom_pos = [
                                data.geom_xpos[idx] - torso_pos for idx in foot_geom_idx
                            ]
                            foot_geom_pos = np.array(foot_geom_pos)
                            rot_mat, z_offset = minimize_z_difference(foot_geom_pos)
                            self.global_z_offset = (
                                -(z_offset + torso_pos[2]) + 0.010
                            ...
                            print(
                                f"Robot {self.robot_name} not supported for calibration"
                            )
            ...
            if self.dump_data:
                np.savez(
                    f"{self.robot_name}_real_data.npz",
                    q=q_data_buffer,
                    qd=qd_data_buffer,
                    ctrl=ctrl_data_buffer,
                )
            if self.use_joystick:
                pygame.quit()
...
if __name__ == "__main__":
    controller = Controller(
        robot_name="h1", use_joystick=True, enable_object_mocap=False
    )
