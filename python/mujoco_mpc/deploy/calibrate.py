...
class Controller:
    def __init__(self, robot_name="g1"):
        ...
        self.global_rpy_offset = np.array([0.0, 0.0, 0.0])
        ...
        try:
            ...
            with mujoco.viewer.launch_passive(
                ...
                while True:
                    ...
                    data.qpos = q_sim
                    data.qvel = qd_sim
                    data.ctrl = np.zeros_like(model.nu)
                    data.time = time.time()
                    ...
                    # get the mean of xmat for left and right foot
                    ...
                    # get its up vector
                    left_foot_up_vec = left_foot_mat[2]
                    right_foot_up_vec = right_foot_mat[2]
                    ...
                    # get sensor foot_right_up
                    # foot_right_up_sensor_name = "foot_right_up"
                    # foot_right_up_sensor_idx = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_SENSOR, foot_right_up_sensor_name)
                    # foot_left_scalar = data.sensordata[model.sensor_adr[foot_right_up_sensor_idx]]
                    # foot_left_up_sensor_name = "foot_left_up"
                    # foot_left_up_sensor_idx = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_SENSOR, foot_left_up_sensor_name)
                    # foot_left_scalar = data.sensordata[model.sensor_adr[foot_left_up_sensor_idx]]
                    # get mean vec

                    
                    # set action
                    ctrl = np.zeros_like(model.nu)
