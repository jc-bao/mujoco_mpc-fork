...
namespace mjpc
{
    std::string Spot2Flat::XmlPath() const
    {
        return GetModelPath("spot2/task_flat.xml");
    }
    std::string Spot2Flat::Name() const { return "Spot2 Flat"; }

    void Spot2Flat::ResidualFn::Residual(const mjModel *model,
                                         const mjData *data,
    ...
    //  ============  transition  ============
    void Spot2Flat::TransitionLocked(mjModel *model, mjData *data)
    {
        // ---------- handle mjData reset ----------
    ...
    void Spot2Flat::ModifyScene(const mjModel *model, const mjData *data,
    ...
    void Spot2Flat::ResetLocked(const mjModel *model)
    {
        // ----------  task identifiers  ----------
        ...
        for (const char *shouldername : {"fl_hip", "hl_hip", "fr_hip", "hl_hip"})
        {
    ...
    void Spot2Flat::ResidualFn::AverageFootPos(
    ...
    double Spot2Flat::ResidualFn::GetPhase(double time) const
    {
        return phase_start_ + (time - phase_start_time_) * phase_velocity_;
    }

    // horizontal Walk trajectory
    void Spot2Flat::ResidualFn::Walk(double pos[2], double time) const
    {
    ...
    // get gait
    Spot2Flat::ResidualFn::A1Gait Spot2Flat::ResidualFn::GetGait() const
    {
        if (current_mode_ == kModeBiped)
            return kGaitTrot;
    ...
    double Spot2Flat::ResidualFn::StepHeight(double time, double footphase,
    ...
    void Spot2Flat::ResidualFn::FootStep(double step[kNumFoot], double time,
                                         A1Gait gait) const
    {
    ...
    // height during flip
    double Spot2Flat::ResidualFn::FlipHeight(double time) const
    {
    ...
    void Spot2Flat::ResidualFn::FlipQuat(double quat[4], double time) const
    {
        double angle = 0;
        if (time >= jump_time_ + flight_time_ + land_time_)
        {
