...
namespace mjpc
{
    std::string Go2Arm::XmlPath() const
    {
        return GetModelPath("go2_arm/task_flat.xml");
    }
    std::string Go2Arm::Name() const { return "Go2 Arm"; }

    void Go2Arm::ResidualFn::Residual(const mjModel *model,
                                      const mjData *data,
    ...
    //  ============  transition  ============
    void Go2Arm::TransitionLocked(mjModel *model, mjData *data)
    {
        // ---------- handle mjData reset ----------
    ...
    void Go2Arm::ModifyScene(const mjModel *model, const mjData *data,
    ...
    void Go2Arm::ResetLocked(const mjModel *model)
    {
        // ----------  task identifiers  ----------
    ...
    void Go2Arm::ResidualFn::AverageFootPos(
    ...
    double Go2Arm::ResidualFn::GetPhase(double time) const
    {
        return phase_start_ + (time - phase_start_time_) * phase_velocity_;
    }

    // horizontal Walk trajectory
    void Go2Arm::ResidualFn::Walk(double pos[2], double time) const
    {
    ...
    // get gait
    Go2Arm::ResidualFn::A1Gait Go2Arm::ResidualFn::GetGait() const
    {
        if (current_mode_ == kModeBiped)
            return kGaitTrot;
    ...
    double Go2Arm::ResidualFn::StepHeight(double time, double footphase,
    ...
    void Go2Arm::ResidualFn::FootStep(double step[kNumFoot], double time,
                                      A1Gait gait) const
    {
    ...
    // height during flip
    double Go2Arm::ResidualFn::FlipHeight(double time) const
    {
    ...
    void Go2Arm::ResidualFn::FlipQuat(double quat[4], double time) const
    {
        double angle = 0;
        if (time >= jump_time_ + flight_time_ + land_time_)
        {
