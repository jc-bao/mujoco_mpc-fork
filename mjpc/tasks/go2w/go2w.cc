...
namespace mjpc
{
  std::string Go2WFlat::XmlPath() const
  {
    return GetModelPath("go2w/task_flat.xml");
  }
  std::string Go2WHill::XmlPath() const
  {
    return GetModelPath("go2w/task_hill.xml");
  }
  std::string Go2WHill::Name() const { return "Go2W Hill"; }
  ...
  //  ============  transition  ============
  void Go2WFlat::TransitionLocked(mjModel *model, mjData *data)
  {
    // ---------- handle mjData reset ----------
    ...
    if (gait_selection != residual_.current_gait_)
    {
      ...
      ResidualFn::Go2WGait gait = residual_.GetGait();
    ...
    // save mode
    residual_.current_mode_ = static_cast<ResidualFn::Go2WMode>(mode);
                            ...
                            mjvScene *scene) const
  {
    ...
    for (ResidualFn::Go2WFoot foot : ResidualFn::kFootAll)
  ...
  void Go2WFlat::ResetLocked(const mjModel *model)
  {
    // ----------  task identifiers  ----------
  ...
  void Go2WFlat::ResidualFn::AverageFootPos(
  ...
  double Go2WFlat::ResidualFn::GetPhase(double time) const
  {
    return phase_start_ + (time - phase_start_time_) * phase_velocity_;
  }

  // horizontal Walk trajectory
  void Go2WFlat::ResidualFn::Walk(double pos[2], double time) const
  {
  ...
  // get gait
  Go2WFlat::ResidualFn::Go2WGait Go2WFlat::ResidualFn::GetGait() const
  {
    if (current_mode_ == kModeBiped)
      return kGaitTrot;
    return static_cast<Go2WGait>(ReinterpretAsInt(current_gait_));
  }
  ...
  // height during flip
  double Go2WFlat::ResidualFn::FlipHeight(double time) const
  {
  ...
  void Go2WFlat::ResidualFn::FlipQuat(double quat[4], double time) const
  {
    double angle = 0;
    if (time >= jump_time_ + flight_time_ + land_time_)
    {
  ...
  // -----------------------------------------------
  void Go2WHill::TransitionLocked(mjModel *model, mjData *data)
  {
