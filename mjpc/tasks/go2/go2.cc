...
namespace mjpc
{
  std::string Go2Flat::XmlPath() const
  {
    return GetModelPath("go2/task_flat.xml");
  }
  std::string Go2Hill::XmlPath() const
  {
    return GetModelPath("go2/task_hill.xml");
  }
  ...
  //  ============  transition  ============
  void Go2Flat::TransitionLocked(mjModel *model, mjData *data)
  {
    // ---------- handle mjData reset ----------
    ...
    if (gait_selection != residual_.current_gait_)
    {
      ...
      ResidualFn::Go2Gait gait = residual_.GetGait();
    ...
    // save mode
    residual_.current_mode_ = static_cast<ResidualFn::Go2Mode>(mode);
  ...
  void Go2Flat::ModifyScene(const mjModel *model, const mjData *data,
                            mjvScene *scene) const
  {
    ...
    for (ResidualFn::Go2Foot foot : ResidualFn::kFootAll)
  ...
  void Go2Flat::ResetLocked(const mjModel *model)
  {
    // ----------  task identifiers  ----------
    ...
    for (const char *footname : {"FL", "HL", "FR", "HR"})
    {
      ...
      if (foot_id < 0)
        mju_error_s("geom '%s' not found", footname);
    ...
    for (const char *shouldername : {"FL_hip", "HL_hip", "FR_hip", "HR_hip"})
    {
      ...
      if (foot_id < 0)
        mju_error_s("body '%s' not found", shouldername);
  ...
  void Go2Flat::ResidualFn::AverageFootPos(
  ...
  double Go2Flat::ResidualFn::GetPhase(double time) const
  {
    return phase_start_ + (time - phase_start_time_) * phase_velocity_;
  }

  // horizontal Walk trajectory
  void Go2Flat::ResidualFn::Walk(double pos[2], double time) const
  {
  ...
  // get gait
  Go2Flat::ResidualFn::Go2Gait Go2Flat::ResidualFn::GetGait() const
  {
    if (current_mode_ == kModeBiped)
      return kGaitTrot;
    return static_cast<Go2Gait>(ReinterpretAsInt(current_gait_));
  }

  // return normalized target step height
  double Go2Flat::ResidualFn::StepHeight(double time, double footphase,
  ...
  void Go2Flat::ResidualFn::FootStep(double step[kNumFoot], double time,
                                     Go2Gait gait) const
  {
  ...
  // height during flip
  double Go2Flat::ResidualFn::FlipHeight(double time) const
  {
  ...
  void Go2Flat::ResidualFn::FlipQuat(double quat[4], double time) const
  {
    double angle = 0;
    if (time >= jump_time_ + flight_time_ + land_time_)
    {
  ...
  // -----------------------------------------------------------------------
  void Go2Hill::ResidualFn::Residual(const mjModel *model,
                                     const mjData *data,
                                     double *residual) const
  {
    // ---------- Residual (0) ----------
  ...
  // -----------------------------------------------
  void Go2Hill::TransitionLocked(mjModel *model, mjData *data)
  {
