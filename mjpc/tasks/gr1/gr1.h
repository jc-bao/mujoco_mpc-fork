...
#ifndef MJPC_TASKS_GR1_GR1_H_
#define MJPC_TASKS_GR1_GR1_H_
...
class GR1Flat : public Task {
 public:
  std::string Name() const override;
  std::string XmlPath() const override;
  class ResidualFn : public mjpc::BaseResidualFn {
   public:
    explicit ResidualFn(const GR1Flat* task)
   ...
   private:
    ...
    constexpr static double kHeightQuadruped = 0.5;  // meter

    // target torso height over feet when bipedal
    constexpr static double kHeightBiped = 0.8;       // meter
    ...
    constexpr static double kLeapHeight = 1.0;        // meter

    // flip: maximum height of flight phase
    constexpr static double kMaxHeight = 1.6;         // meter
...
class GR1Hill : public Task {
 public:
  std::string Name() const override;
  std::string XmlPath() const override;
  class ResidualFn : public mjpc::BaseResidualFn {
   public:
    explicit ResidualFn(const GR1Hill* task, int current_mode = 0)
    ...
    void Residual(const mjModel* model, const mjData* data,
                  double* residual) const override;
   private:
    friend class GR1Hill;
    int current_mode_;
  };
  GR1Hill() : residual_(this) {}
  void TransitionLocked(mjModel* model, mjData* data) override;
