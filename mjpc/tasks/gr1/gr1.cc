...
namespace mjpc
{
    std::string GR1Hill::XmlPath() const
    {
        return GetModelPath("gr1/task_hill.xml");
    }
    std::string GR1Flat::XmlPath() const
    {
        return GetModelPath("gr1/task_flat.xml");
    }
    std::string GR1Hill::Name() const { return "GR1 Hill"; }
    std::string GR1Flat::Name() const { return "GR1 Flat"; }

    void GR1Flat::ResidualFn::Residual(const mjModel *model,
                                       const mjData *data,
                                       double *residual) const
    {
        ...
        if (current_mode_ != kModeFlip)
        {
            if (current_mode_ == kModeBiped)
            {
                ...
                residual[counter++] = torso_xmat[8] - handstand;
            }
            else
            {
                // residual[counter++] = torso_xmat[8] - 1;
                residual[counter++] = torso_xmat[6] + 1;
            }
            // }
            residual[counter++] = 0;
            residual[counter++] = 0;
    ...
    //  ============  transition  ============
    void GR1Flat::TransitionLocked(mjModel *model, mjData *data)
    {
        // ---------- handle mjData reset ----------
    ...
    void GR1Flat::ModifyScene(const mjModel *model, const mjData *data,
    ...
    void GR1Flat::ResetLocked(const mjModel *model)
    {
        // ----------  task identifiers  ----------
    ...
    void GR1Flat::ResidualFn::AverageFootPos(
    ...
    double GR1Flat::ResidualFn::GetPhase(double time) const
    {
        return phase_start_ + (time - phase_start_time_) * phase_velocity_;
    }

    // horizontal Walk trajectory
    void GR1Flat::ResidualFn::Walk(double pos[2], double time) const
    {
    ...
    // get gait
    GR1Flat::ResidualFn::A1Gait GR1Flat::ResidualFn::GetGait() const
    {
        if (current_mode_ == kModeBiped)
            return kGaitTrot;
    ...
    double GR1Flat::ResidualFn::StepHeight(double time, double footphase,
    ...
    void GR1Flat::ResidualFn::FootStep(double step[kNumFoot], double time,
                                       A1Gait gait) const
    {
    ...
    // height during flip
    double GR1Flat::ResidualFn::FlipHeight(double time) const
    {
    ...
    void GR1Flat::ResidualFn::FlipQuat(double quat[4], double time) const
    {
        double angle = 0;
        if (time >= jump_time_ + flight_time_ + land_time_)
        {
    ...
    // -----------------------------------------------------------------------
    void GR1Hill::ResidualFn::Residual(const mjModel *model,
                                       const mjData *data,
                                       double *residual) const
    {
        // ---------- Residual (0) ----------
    ...
    // -----------------------------------------------
    void GR1Hill::TransitionLocked(mjModel *model, mjData *data)
    {
